...
## 8. Mobile-first multi-page workspace

I rebuilt the local workspace as focused pages instead of one crowded screen. The workspace now has independent HTML pages for Assistant, Knowledge Canvas, Practice, Code Lab, Mind Map, Progress, and Settings. Each page uses the same mobile-first shell, navigation, header, shared stylesheet, and browser state.

The page files are generated by `build_app_pages.py`, while `app.js` now checks which controls exist before binding events. This means each page can load independently without requiring every page’s controls to be present. On small screens, the sidebar becomes a slide-in navigation drawer and each page keeps one clear task in view. On larger screens, the same pages use the desktop sidebar and wider content layout.
